...
impl Location {
    ...
    pub fn distance(&self, other: &Location) -> f64 { self.distance2(other).sqrt() }

    pub fn h_distance2(&self, other: &Location) -> f64 {
        (other.x - self.x).powf(2.0) + (other.z - self.z).powf(2.0)
    }

    pub fn h_distance(&self, other: &Location) -> f64 { self.h_distance2(other).sqrt() }

    pub fn yaw_angle(&self) -> Angle { (self.yaw * 256f32 / 360f32).rem_euclid(256f32) as Angle }
