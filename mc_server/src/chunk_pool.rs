...
impl<T: 'static+ChunkGenerator> ChunkPool<T> {
    ...
    pub fn update_chunk(&mut self, x: i32, z: i32) { self.chunks_to_update.push((x, z)); }
    ...
    pub async fn add_player(&mut self, player: Arc<RwLock<BoxedEntity>>) {
        ...
        self.synced_player_chunks
            .insert(eid, (location.chunk_x(), location.chunk_z()));
    }
                ...
                match chunks.read().await.get(&(chunk_x + dx, chunk_z + dz)) {
                    ...
                    None => {
                        ...
                        tokio::task::spawn(async move {
                            ...
                            chunk.data = chunk_generator
                                .generate_chunk_data(chunk_x + dx, chunk_z + dz)
                                .await;
                            let chunk = Arc::new(RwLock::new(chunk));
                            chunks
                                .write()
                                .await
                                .insert((chunk_x + dx, chunk_z + dz), Arc::clone(&chunk));
                            player
                                .send_packet(&chunk.read().await.encode(true))
                                .await
                                .unwrap();
    ...
    pub async fn tick(&mut self) -> Result<()> {
        ...
        for (eid, player) in players {
            ...
            if synced_chunk
                ...
                .unwrap_or(true)
            {
                self.update_player_view_position(eid, current_chunk.0, current_chunk.1)
                    .await;
